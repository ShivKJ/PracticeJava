...
public abstract class Tree<K extends Comparable<K>, V> {
	...
	public static <T extends Node<K, V>, K extends Comparable<K>, V> T min(T n) {
		while (n.l() != nil())
			n = n.l();
		return n;
	}

	public static <K extends Comparable<K>, V> void inorder(Node<K, V> n, Collection<Entry<K, V>> coll) {
		if (n.l() != nil())
			inorder(n.l(), coll);
